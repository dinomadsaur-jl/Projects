...
Fixed verbose logging - ALL print statements are now captured!
"""
...
# ══════════════════════════════════════════════════════════════════
#  PWA GENERATOR - Fixed verbose logging
# ══════════════════════════════════════════════════════════════════
class PyodidePWA:
...
<body>
    ...
    <div class="loading" id="loading">
        ...
        <div id="loadingDetails" style="font-size:12px; margin-top:10px; color:#aaa;"></div>
    </div>
    ...
    <script>
        ...
        // Add output line
        function addOutputLine(text, type = 'print') {{
            if (!verbose && type === 'print') return;
        ...
        function updateOutput() {{
            ...
            outputLines.forEach(line => {{
                ...
                if (line.type === 'html') {{
                    ...
                    html += '<div class="console-error">❌ ' + escaped + '</div>';
                }} else if (line.type === 'system') {{
                    html += '<div class="console-line"><span class="console-timestamp">[' + line.timestamp + ']</span> ' +
                           '<span class="console-system">🔧 ' + escaped + '</span></div>';
                }} else {{
                    html += '<div class="console-line"><span class="console-timestamp">[' + line.timestamp + ']</span> ' +
                           '<span class="console-print">📝 ' + escaped + '</span></div>';
                }}
            }});
        ...
        async function init() {{
            ...
            addOutputLine('🚀 PyWA starting...', 'system');
            ...
            try {{
                ...
                if (PACKAGES.length > 0) {{
                    ...
                    for (const pkg of PACKAGES) {{
                        ...
                        try {{
                            ...
                            addOutputLine(`⚠️ Failed to install ${{pkg}}: ${{e}}`, 'error');
                        }}
                    }}
                }}
                ...
                addOutputLine(`❌ Init failed: ${{error}}`, 'error');
        ...
        async function runPython() {{
            ...
            addOutputLine('\\n' + '='.repeat(50), 'system');
            addOutputLine('🚀 Executing Python...', 'system');
...
import sys
import io
from js import addOutputLine

# Custom stdout that sends everything to JavaScript
class PyWAOutput:
    def write(self, text):
...
sys.stdout = PyWAOutput()
sys.stderr = PyWAOutput()

print("✅ Python output capture enabled")
                `);
                
                // Run user code
                await pyodide.runPythonAsync(PYTHON_CODE);
                
                addOutputLine('✅ Execution complete', 'system');
                addOutputLine('='.repeat(50), 'system');
    ...
    def generate(cls, python_file, config):
        ...
        print(f"📦 Packages: {packages if packages else 'none'}")
        
        # Wrap code with CORS proxy
        wrapped_code = f'''
# PyWA - Python code
import sys
import requests
