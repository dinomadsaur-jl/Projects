...
AUTOMATICALLY OPENS YOUR BROWSER for installation!
"""
...
# ══════════════════════════════════════════════════════════════════
#  SERVER FUNCTIONS - FIXED TO OPEN BROWSER
# ══════════════════════════════════════════════════════════════════
...
def serve_and_open(directory, port):
    """Start server in background thread and open browser."""
    # Start server in background
    server = ServerThread(directory, port)
    server.start()
    ...
    print("  📱 Opening browser for installation...")
    ...
    # Method 3: print URL for manual opening
    if not opened:
        print(f"  ⚠️  Could not open browser automatically")
        print(f"  📍 Please open this URL manually: {url}")
    
    print("\n  💡 To install as PWA:")
    print("     • Chrome/Edge: Click install icon in address bar")
    print("     • Safari: Share → Add to Home Screen")
    print("     • Firefox: Not supported for installation")
...
# ══════════════════════════════════════════════════════════════════
#  MAIN CLI - AUTOMATICALLY OPENS BROWSER
# ══════════════════════════════════════════════════════════════════
def main():
    parser = argparse.ArgumentParser(
        prog="pywa",
        description="PyWA — Compile Python to PWA (automatically opens browser for installation)"
    )
    ...
    parser.add_argument("--no-serve", action="store_true", help="Compile only, don't start server")
    ...
    # Serve and open browser (unless --no-serve)
    if not args.no_serve:
        port = args.port or find_free_port(8080)
        serve_and_open(out_dir, port)
    else:
        print(f"\n  📁 PWA files are in ./{args.out}/")
        print("  To serve and install, run without --no-serve")
