...
class HTMLCapturer:
        ...
        try:
            ...
            if new_html:
                ...
                if '<html' in html_content.lower() or '<!DOCTYPE' in html_content.upper():
                    ...
                    print(f"     ⚠️  File exists but may not be complete HTML")
                    return html_content, True, result.returncode
            ...
            # Check if stdout contains a filename that was created
            lines = result.stdout.split('\n')
            for line in lines:
                if '.html' in line and 'saved' in line.lower():
                    # Try to extract filename
                    match = re.search(r'([a-zA-Z0-9_\-]+\.html)', line)
                    if match:
                        possible_file = Path(file_path.parent) / match.group(1)
                        if possible_file.exists():
                            html_content = possible_file.read_text(encoding='utf-8')
                            print(f"     ✅ Captured from referenced file: {possible_file.name}")
                            return html_content, True, result.returncode
    ...
    def _is_html(text):
        ...
        html_patterns = [
            ...
            'Plotly', 'chart', 'dashboard', 'margin:',
            'padding:', 'background:', 'font-family:'
        ]
        ...
        for pattern in html_patterns:
            if pattern.lower() in text_lower:
                score += 1
        return score >= 3  # Need at least 3 indicators to be considered HTML
