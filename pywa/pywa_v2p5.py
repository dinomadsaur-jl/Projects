...
Fixed version with verbose logging and proper error handling.
"""
...
def detect_packages(code):
    ...
    package_map = {
        'numpy': ['numpy', 'np'],
        'pandas': ['pandas', 'pd'],
        'plotly': ['plotly', 'px', 'go', 'plotly.graph_objects', 'plotly.express'],
        'scipy': ['scipy'],
        'requests': ['requests'],
...
# ══════════════════════════════════════════════════════════════════
#  PWA GENERATOR - Fixed version with verbose logging
# ══════════════════════════════════════════════════════════════════
class PyodidePWA:
...
<body>
    <div class="app-header">
        ...
        <div class="header-controls">
            ...
            <button class="refresh-btn" onclick="toggleVerbose()">📋 <span id="verboseStatus">ON</span></button>
        </div>
    </div>
    ...
    <script>
        ...
        function toggleVerbose() {{
            ...
            addOutputLine(verbose ? '📢 Verbose logging enabled' : '🔇 Verbose logging disabled', 'system');
        }}
        ...
        function clearOutput() {{
            outputLines = [];
            updateOutput();
            addOutputLine('🗑️ Output cleared', 'system');
        }}
            ...
            outputLines.forEach(line => {{
                ...
                if (line.type === 'html') {{
                    ...
                    html += '<div class="console-line"><span class="console-timestamp">[' + line.timestamp + ']</span> ' + 
                           '<span style="color: #8888ff;">🔧 ' + escapedText + '</span></div>';
                }} else if (line.type === 'print') {{
                    html += '<div class="console-line"><span class="console-timestamp">[' + line.timestamp + ']</span> ' + 
                           '<span class="console-output-print">📝 ' + escapedText + '</span></div>';
                }} else {{
                    html += '<div class="console-line"><span class="console-timestamp">[' + line.timestamp + ']</span> ' + 
        ...
        function retryLoading() {{
            loadingAttempts++;
            addOutputLine('🔄 Retrying initialization (attempt ' + (loadingAttempts + 1) + ')...', 'system');
            init();
        }}
        ...
        async function init() {{
            ...
            addOutputLine('🚀 PyWA starting up...', 'system');
            addOutputLine('📱 Device: ' + navigator.userAgent, 'system');
            addOutputLine('🌐 Network: ' + (navigator.onLine ? 'Online' : 'Offline'), 'system');
            
            if (PACKAGES.length > 0) {{
                addOutputLine('📦 Required packages: ' + PACKAGES.join(', '), 'system');
                packageStatus.innerHTML = '📦 Packages: ' + PACKAGES.join(', ');
            }}
            
            try {{
                status.textContent = 'Loading Pyodide...';
                addOutputLine('📦 Loading Pyodide...', 'system');
                
                pyodide = await loadPyodide({{
                    indexURL: "https://cdn.jsdelivr.net/pyodide/v0.24.1/full/",
                }});
                
                addOutputLine('✅ Pyodide loaded', 'system');
                status.textContent = 'Pyodide loaded';
                
                if (PACKAGES.length > 0) {{
                    status.textContent = 'Loading packages...';
                    addOutputLine('📚 Loading micropip...', 'system');
                    ...
                    for (const pkg of PACKAGES) {{
                        ...
                        addOutputLine(`📦 Installing ${{pkg}}...`, 'system');
                        
                        try {{
                            await micropip.install(pkg);
                            addOutputLine(`✅ ${{pkg}} installed`, 'system');
                        }} catch (e) {{
                    ...
                    addOutputLine('✅ All packages installed', 'system');
                }}
                
                isReady = true;
                status.textContent = 'Ready!';
                addOutputLine('✅ Python environment ready', 'system');
        ...
        async function runPython() {{
            ...
            addOutputLine('\\n' + '='.repeat(50), 'system');
            addOutputLine('🚀 Starting Python execution', 'system');
...
print("PyWA: Python environment ready")
                ...
                addOutputLine('📝 Executing user code...', 'system');
                addOutputLine('-' .repeat(40), 'system');
                
                await pyodide.runPythonAsync(PYTHON_CODE);
                
                addOutputLine('-' .repeat(40), 'system');
                addOutputLine('✅ Python execution complete', 'system');
                addOutputLine('='.repeat(50), 'system');
...
def serve_directory(directory, port):
    ...
    try:
        ...
        print(f"\n📝 All print statements will appear in the PWA")
...
# ══════════════════════════════════════════════════════════════════
def main():
    parser = argparse.ArgumentParser(description="PyWA - Python to PWA Compiler")
    ...
    parser.add_argument("--color", default="#1a1a2e", help="Primary color (hex)")
    parser.add_argument("--out", default="pwa_output", help="Output directory")
    parser.add_argument("--no-serve", action="store_true", help="Compile only, don't serve")
    ...
    if not args.no_serve:
        ...
        print("Run without --no-serve to start server")
