    ...
    if spy_df is not None and not spy_df.empty:
        try:
            ...
            if aligned.notna().any():
                ...
                # Changed to 3 months (\~63 trading days)
...
def determine_trend_state(df):
    ...
    price = cur['Close']
    sma50 = cur['SMA_50']
    rel = cur.get('Rel_Perf_3M', 0)  # Now uses 3M rel perf
...
def generate_dashboard(force_refresh=False):
    ...
    for key, config in SECTORS.items():
        ...
        sectors_data.append({
            ...
            'return': last['Return'],
            'alpha': last['Alpha'],
            'rel_perf': last['Rel_Perf_3M'],  # Now using 3M
    ...
    # Sort by worst alpha first (lowest/negative on top)
...
<body>
    ...
    <div class="update">Updated: {timestamp} • Sorted: Worst Alpha → Best • 3M Rel Perf • Click for 5Y + SMA50 Reversals</div>
...
"""
    ...
    print("   → Sorted by worst Alpha first (negative/lowest on top)")
    print("   → Using 3-month relative performance (Rel_Perf_3M)")
