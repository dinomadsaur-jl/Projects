...
# Colors
COLOR_GOLD = '#D4AF37'
COLOR_SILVER = '#708090'      # ← clearer slate gray from your Matplotlib version
COLOR_RATIO = '#800080'
COLOR_DXY = '#003366'
COLOR_CORR = '#FF6347'
...
def run_analysis():
    ...
    # Silver (right, shared with Ratio) – raw faint + SMA solid
    fig.add_trace(
    ...
    # Correlation status – fixed overlay between plots (paper coordinates)
    corr_status = "Decoupled" if abs(last_corr) < 0.3 else ("Inverse" if last_corr < 0 else "Direct")
    fig.add_shape(
        type="rect",
        xref="paper", yref="paper",
        x0=0.02, y0=0.45, x1=0.30, y1=0.55,  # centered vertically between plots
        fillcolor="white", opacity=0.95,
        line=dict(color="black", width=1.5),
        layer="above"
    )
    fig.add_annotation(
        x=0.16, y=0.50, xref="paper", yref="paper",
    ...
    # ────────────────────────────────────────────────
    # Layout – optimized for full screen portrait/landscape, strict 5-year view
    # ────────────────────────────────────────────────
    fig.update_layout(
        ...
        xaxis_range=[cutoff, now],  # strict 5-year zoom
        xaxis=dict(fixedrange=False)  # allow horizontal zoom/pan but start at 5 years
    )
    ...
    # DXY y-axis starts from lowest value (dynamic)
    dxy_min = df['DXY'].min() * 0.95 if 'DXY' in df else 80
    dxy_max = df['DXY'].max() * 1.05 if 'DXY' in df else 120
    fig.update_yaxes(range=[dxy_min, dxy_max], row=2, col=1, secondary_y=False)
