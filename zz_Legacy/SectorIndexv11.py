    ...
    if spy_df is not None and not spy_df.empty:
        try:
            ...
            if aligned.notna().any():
                ...
                df['Alpha'] = df['Alpha'].fillna(0)  # Ensure no None/NaN
                return df
        except:
            pass
    df['Alpha'] = 0
...
<body>
    ...
    <div class="update">Updated: {timestamp} • Sorted: Worst Alpha → Best • 3M Rel Perf • Click for 5Y + SMA20 + Alpha subplot</div>
    ...
    <script>
        ...
        function showPlot(s) {{
            ...
            console.log("SMA20 length:", s.plot_data.sma20 ? s.plot_data.sma20.length : 0);
            console.log("Reversals:", s.plot_data.reversals ? s.plot_data.reversals.length : 0);

            document.getElementById('modal-title').textContent = 
                s.config.name + " (" + s.config.ticker + ") - 5 Years + SMA20 (colored) + Alpha Subplot";
            ...
            // SMA 20 (colored by direction - green rising, red falling)
            if (d.dates && d.sma20 && d.sma20.length === d.dates.length) {{
                const smaColors = [];
                for (let i = 1; i < d.sma20.length; i++) {{
                    smaColors.push(d.sma20[i] > d.sma20[i-1] ? '#2ca02c' : '#d62728');
                }}
                smaColors.unshift(smaColors[0] || '#2ca02c');  // first point same as second
...
"""
    ...
    print("   → Alpha now in separate subplot (bottom)")
    print("   → SMA 20 colored: green (rising) / red (falling)")
    print("   → Sorted by worst Alpha first")
