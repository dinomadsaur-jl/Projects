...
def fetch_yahoo_data(ticker, start_date, end_date, interval):
    try:
        ...
        url = f"https://query2.finance.yahoo.com/v8/finance/chart/{ticker}"  # ← changed to query2
...
def run_analysis():
    ...
    # ────────────────────────────────────────────────
    # VISUALIZATION (your original simpler layout)
    # ────────────────────────────────────────────────
    fig = make_subplots(
        rows=2, cols=1,
        shared_xaxes=True,
    ...
    fig.add_trace(go.Scatter(x=df.index, y=df['Silver'], name="Silver ($)", line=dict(color=COLOR_SILVER, width=1),
                             visible='legendonly'),
                  row=1, col=1, secondary_y=False)
    ...
    current_corr = df['Corr'].iloc[-1] if not df['Corr'].empty else np.nan
