...
def fetch_yahoo_data(ticker, days_back):
    ...
    Switches to '1wk' interval if days_back > 1000.
    """
    # Determine interval and buffer based on duration
    if days_back > 1000:
        interval = "1wk"
        # Buffer needs to be larger for weekly SMA (20 weeks ~ 140 days)
        # We add 250 days to be safe
        buffer_days = 250
    else:
        interval = "1d"
        buffer_days = 100
    ...
    start_date = end_date - timedelta(days=days_back + buffer_days)
...
def run_analysis(period_name, days_count):
    ...
    # Formatting
    chart_freq = "Weekly" if days_count > 1000 else "Daily"
    ax1.set_title(f"Market Analysis ({period_name}) - {chart_freq} SMA Direction Changes", fontsize=16)
