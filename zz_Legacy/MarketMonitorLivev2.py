...
def fetch_yahoo_data(ticker, start_date, end_date, interval):
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"HTTP Error {getattr(e.response, 'status_code', 'unknown')} for {ticker} ({interval})")
...
def run_analysis_and_serve(days_count):
    ...
    fig = make_subplots(
        rows=2, cols=1,
        shared_xaxes=True,
        row_heights=[0.55, 0.45],          # more space for DXY in portrait
    ...
    # Gold
    g_price, g_date = last_valid(df['Gold'])
    if not np.isnan(g_price):
        text = f"Gold {g_price:,.0f}"
        if g_date and g_date.strftime("%b %d") != today_str:
            text += f" ({g_date.strftime('%b %d')})"
        fig.add_annotation(
            x=last_idx, y=g_price,
            text=text,
    ...
    # Silver
    s_price, s_date = last_valid(df['Silver'])
    if not np.isnan(s_price):
        text = f"Silver {s_price:,.2f}"
        if s_date and s_date.strftime("%b %d") != today_str:
            text += f" ({s_date.strftime('%b %d')})"
        fig.add_annotation(
            x=last_idx, y=s_price,
            text=text,
            xanchor="left", ax=40, ay=0,
            font=dict(color='#B0C4DE', size=13),
            bgcolor="rgba(255,255,255,0.85)",
            bordercolor="#B0C4DE", borderwidth=1,
            showarrow=False,
            row=1, col=1,
            secondary_y=True
        )
    ...
    # DXY
    d_price, d_date = last_valid(df['DXY'])
    if not np.isnan(d_price):
        text = f"DXY {d_price:,.2f}"
        if d_date and d_date.strftime("%b %d") != today_str:
            text += f" ({d_date.strftime('%b %d')})"
        fig.add_annotation(
            x=last_idx, y=d_price,
            text=text,
            xanchor="left", ax=40, ay=0,
            font=dict(color='#4169E1', size=13),
            bgcolor="rgba(255,255,255,0.85)",
            bordercolor="#4169E1", borderwidth=1,
            showarrow=False,
            row=2, col=1
        )
    ...
    fig.update_layout(
        ...
        autosize=True,
        title=dict(text="Gold / Silver / DXY – 10yr View (2yr Corr)", font=dict(size=15)),
