...
def load_cached_data(ticker):
    ...
    try:
        ...
        if age_hours > CACHE_HOURS:
            print(f"Cache expired for {ticker} ({age_hours:.1f}h old)")
            return None
        
        if df is None or df.empty or len(df) < MIN_BARS_REQUIRED:
            print(f"Cache invalid for {ticker}: too few bars")
            return None
        
        first_date = df.index.min()
        years = (datetime.now().date() - first_date.date()).days / 365.25
        if years < 4.7 or len(df) < EXPECTED_MIN_BARS_5Y:
            print(f"Cache for {ticker} does not cover 5 years ({years:.1f} years)")
            return None
        
        last_date = df.index.max()
        if (datetime.now().date() - last_date.date()).days > 7:
            print(f"Cache stale for {ticker}: last bar >7 days old")
            return None
        
        print(f"✓ Valid cache for {ticker} ({len(df)} bars)")
        return df
...
def get_stock_data(ticker, force_refresh=False):
    ...
    ua_list = [
        ...
        'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 Chrome/129.0.0.0 Mobile Safari/537.36',
    ]
...
def generate_dashboard(force_refresh=False):
    ...
    for key, config in SECTORS.items():
        ...
        # SMA 50 slope (direction change detection)
        df['SMA50_Slope'] = df['SMA_50'].diff().fillna(0)
        df['SMA50_Direction'] = np.sign(df['SMA50_Slope'])

        # Find points where direction changes
        df['Direction_Change'] = df['SMA50_Direction'] != df['SMA50_Direction'].shift(1)

        # Filter meaningful reversals with price move ≥1.5% from recent extreme
        reversals = []
        for i in df[df['Direction_Change']].index:
            if i == df.index[0]: continue
            recent_slice = df.loc[:i].tail(20)  # look back \~1 month
            if recent_slice.empty: continue

            if df.at[i, 'SMA50_Direction'] > 0:  # Bullish reversal
                recent_low = recent_slice['Close'].min()
                price_move = (df.at[i, 'Close'] - recent_low) / recent_low * 100
                if price_move >= 1.5:
                    reversals.append({
                        'date': i.strftime('%Y-%m-%d'),
                        'price': df.at[i, 'Close'],
                        'signal': 'Bull',
                        'move': round(price_move, 2)
                    })
            else:  # Bearish reversal
                recent_high = recent_slice['Close'].max()
                price_move = (df.at[i, 'Close'] - recent_high) / recent_high * 100
                if price_move <= -1.5:
                    reversals.append({
                        'date': i.strftime('%Y-%m-%d'),
                        'price': df.at[i, 'Close'],
        ...
        plot_data = {
            ...
            'sma20': df['SMA_20'].replace([np.inf, -np.inf, np.nan], None).round(2).tolist(),
    ...
    sectors_data.sort(key=lambda x: x.get('rel_perf', 0))  # Lagging first
...
<body>
    ...
    <div class="update">Updated: {timestamp} • Sorted: Lagging → Leading • 150D Rel Perf • Click for 5Y + SMA50 Direction Reversals</div>
    ...
    <script>
        ...
        function showPlot(s) {{
            ...
            console.log("Data lengths:", {{
                dates: s.plot_data.dates.length,
                close: s.plot_data.close.length,
                sma50: s.plot_data.sma50.length,
                reversals: s.plot_data.reversals.length
            }});
            ...
            // Reversal annotations with price labels
            ...
            if (traces.length === 0) {{
                console.error("No valid data for plot");
                return;
            }}
...
"""
    ...
    print("Charts should now show SMA50 + direction-based reversals with price annotations")


if __name__ == "__main__":
    generate_dashboard(force_refresh=False)  # Change to True to force fresh 5-year data