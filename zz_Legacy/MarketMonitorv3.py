...
THRESHOLD = 0.02  # 2% Filter for reversal points
...
def get_data(ticker, days):
    ...
    # Use weekly data for the 5-year chart to maintain performance
    ...
    try:
        ...
        return {datetime.fromtimestamp(ts).strftime('%Y-%m-%d'): c for ts, c in zip(res['timestamp'], res['indicators']['quote'][0]['close']) if c is not None}
    except: return {}
...
def find_significant_reversals(data_list, threshold=THRESHOLD):
    """Detects reversals only if change exceeds the percentage threshold."""
    reversals = []
    ...
    for i in range(1, len(data_list)):
        ...
        if trend <= 0 and pct_change >= threshold: # Confirm Uptrend (Trough found)
            reversals.append((i, 'trough'))
            last_extreme_val = price
            trend = 1
        elif trend >= 0 and pct_change <= -threshold: # Confirm Downtrend (Peak found)
            reversals.append((i, 'peak'))
            last_extreme_val = price
            trend = -1
        elif (trend == 1 and price > last_extreme_val) or (trend == -1 and price < last_extreme_val):
            last_extreme_val = price # Update the extreme point
...
def plot_market(period_name, days_count):
    ...
    dates_str = sorted(set(hist['Gold'].keys()) & set(hist['Silver'].keys()) & set(hist['DXY'].keys()))[-days_count:]
    
    dates = [datetime.strptime(d, '%Y-%m-%d') for d in dates_str]
    g, s, dxy = [[hist[n][d] for d in dates_str] for n in ['Gold', 'Silver', 'DXY']]
    ratio = [gv/sv for gv, sv in zip(g, s)]
    
    win = 20 if days_count < 1000 else 10 # SMA window
    ...
    ax1.set_title(f"Significant Trend Reversals ({period_name.upper()}) - {THRESHOLD*100}% Filter", fontsize=16, pad=20)
    ax1.xaxis.set_major_formatter(mdates.DateFormatter('%b %Y' if days_count < 500 else '%Y'))
    
    path = os.path.join(SAVE_DIR, f"Market_{period_name}.jpg")
...
if __name__ == "__main__":
    ...
    print(f"\nSaved 3 reports to {SAVE_DIR}. Opening 6-month view...")
