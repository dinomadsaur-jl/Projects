...
Usage:
    ...
    python pywa.py build ../projects/myapp.py          # relative paths work
    python pywa.py build /home/user/myapp.py           # absolute paths work
    python pywa.py build                                # interactive picker
...
# ═════════════════════════════════
# ══════════════════════════════════════════════════════════════════
#  SIMPLIFIED FILE PICKER  (works everywhere, no complex terminal handling)
# ══════════════════════════════════════════════════════════════════
# ══════════════════════════════════════════════════════════════════
#  SIMPLIFIED FILE PICKER  (works everywhere, no complex terminal handling)
# ══════════════════════════════════════════════════════════════════
...
def _pick_from_list(files: list) -> Path | None:
    """
    Display a numbered list of files and let user pick one.
    ...
    for i, f in enumerate(files, 1):
        try:
            # Show path relative to current directory when possible
...
def pick_py_file(start_dir: Path = None, recursive: bool = True) -> Path | None:
    """
    Simple, reliable file picker that works everywhere.
    Uses a numbered menu system that works in any terminal.
    """
    ...
    if not found:
        print(f"\n  ❌ No Python files found in {base}")
        
        # Offer to search from parent
        if recursive and base.parent != base:
            try:
                ans = input("\n  Search in parent directory? [y/N]: ").strip().lower()
                if ans in ('y', 'yes'):
                    return pick_py_file(base.parent, recursive=True)
            except (EOFError, KeyboardInterrupt):
                pass
        return None
...
def resolve_file_with_path(file_arg: str = None) -> Path | None:
    """
    Resolve a file path with improved handling.
    """
    # No argument - launch picker
    if not file_arg:
        return pick_py_file()
    ...
    if path.exists():
        if path.is_file() and path.suffix == '.py':
            ...
            print(f"\n  📁 '{file_arg}' is a directory. Searching inside...")
            return pick_py_file(path)
        else:
            print(f"\n  ❌ '{file_arg}' is not a Python file")
            return None
    ...
    if not path.suffix:
        cwd_path_with_ext = Path.cwd() / path.with_suffix('.py')
        if cwd_path_with_ext.exists():
            return cwd_path_with_ext
    
    # Try searching for the filename
    if len(path.parts) == 1:  # Just a filename
        print(f"\n  🔍 Searching for '{path.name}'...")
        
        matches = []
        for f in Path.cwd().rglob(f"**/{path.name}"):
        ...
        if len(matches) == 1:
            ...
            print(f"\n  Found multiple files named '{path.name}':")
    ...
    print("    • Use full path: /path/to/your/file.py")
    print("    • Run without arguments to use file picker: python pywa.py build")
    
    return None

# For backward compatibility with any code that might call these
_pick_numbered = _pick_from_list
_pick_with_filter = _pick_from_list
...
def pick_py_file(start_dir: Path = None, recursive: bool = True) -> Path | None:
    ...
    # Show results
    print(f"\n  📁 Found {len(found)} Python file(s):\n")
    
    # Display with numbers (show relative paths when possible)
...
def resolve_file_with_path(file_arg: str = None) -> Path | None:
    ...
    if len(path.parts) == 1:  # Just a filename
        ...
        if len(matches) == 1:
            ...
            print(f"\n  Found multiple files named '{path.name}':")
            return pick_py_file(start_dir=matches[0].parent)
...
def _pick_numbered(files: list) -> Path | None:
    """Simple numbered list picker for non-TTY environments."""
    print("\n  Available .py files:")
    for i, f in enumerate(files, 1):
        try:
            display = f.relative_to(Path.cwd())
...
def pick_py_file(start_dir: Path = None, recursive: bool = True) -> Path | None:
    ...
    Can search recursively from the starting directory.
    
    Args:
        start_dir: Directory to start searching from (default: current working dir)
    ...
    # Collect candidates — skip pywa.py, setup.py, conftest.py, and common dirs to ignore
    skip_names = {"pywa.py", "setup.py", "conftest.py", "test_*.py"}
    skip_dirs  = {
        "__pycache__", ".git", ".github", ".vscode", ".idea", 
        "venv", ".venv", "env", ".env", "node_modules", 
        "dist", "build", "*.egg-info", ".pytest_cache", ".mypy_cache"
    ...
    if recursive:
        # Recursive search
        pattern = "**/*.py"
        search_desc = f"recursively in {base}"
    else:
        # Only current directory
        pattern = "*.py"
        search_desc = f"in {base} (non-recursive)"
    
    print(f"  📁 Searching {search_desc}")
    
    # Do the search
    for f in sorted(base.glob(pattern)):
        # Skip if it's a directory (shouldn't happen with .py pattern)
        if not f.is_file():
            continue
            
        # Skip files with names we want to ignore
        if any(f.name == skip or f.match(skip) for skip in skip_names):
            continue
        ...
        # Skip if it's in a hidden directory (starts with .) except current dir
        if any(part.startswith('.') and part != '.' for part in f.parts[:-1]):
            continue
            
        found.append(f)
    
    if not found:
        print(f"\n  ❌ No Python app files found in {base}")
        print("  Make sure your app file is in the current directory or")
        print("  specify the full path: python pywa.py build /path/to/your/app.py\n")
        
        # Offer to search from parent directory
        if recursive and base.parent != base:
            try:
                ans = input("  Search in parent directory? [y/N]: ").strip().lower()
                if ans in ('y', 'yes'):
                    return pick_py_file(base.parent, recursive=True)
            except EOFError:
                pass
    ...
    if len(found) == 1:
        try:
            display = found[0].relative_to(Path.cwd())
        except ValueError:
            display = found[0]
        print(f"\n  📄 Found 1 file: {display}")
    else:
        print(f"\n  📄 Found {len(found)} Python files")
    
    # ── Auto-select if only one file ──────────────────────────
    if len(found) == 1:
        print(f"  Auto-selected: {found[0].name}\n")
        return found[0]
    ...
    return _pick_with_filter(found)


def _pick_with_filter(files: list) -> Path | None:
    """Interactive file picker with filtering for many files."""
    print(f"\n  Found {len(files)} .py files. You can:")
    print("    1. Type part of the filename to filter")
    print("    2. Press Tab to auto-complete")
    print("    3. Press Enter with empty input to see all files")
    print()
    ...
    try:
        ...
        try:
            ...
            for f in files:
                try:
                    names.append(str(f.relative_to(Path.cwd())))
                except ValueError:
                    names.append(str(f))
        ...
        user_input = input("  Filename or pattern: ").strip()
        
        if not user_input:
            # Show all files with numbers
            return _pick_from_list(files)
        
        # Try exact match first
        for f in files:
            if f.name == user_input or f.stem == user_input:
                return f
            # Also try matching against full path
            if str(f) == user_input or str(f.absolute()) == user_input:
                return f
        ...
        matches = []
        for f in files:
            if user_lower in f.name.lower() or user_lower in str(f).lower():
                matches.append(f)
...
def resolve_file_with_path(file_arg: str = None) -> Path | None:
    """
    Resolve a file path, with improved path handling.
    Can handle:
      - Relative paths (myapp.py, ./myapp.py, ../project/app.py)
      - Absolute paths (/home/user/app.py, C:\\Users\\app.py)
      - Just filenames (search in current dir and subdirs)
      - None (launch interactive picker)
    """
    if not file_arg:
        return pick_py_file()
    ...
    # Try as a relative path without current dir prefix
    if path.exists():
        return path
    
    # If it's just a filename, search for it in current directory and subdirs
    if len(path.parts) == 1:  # Just a filename like "myapp.py"
        print(f"  🔍 Searching for '{path.name}' in current directory...")
        
        # Search recursively for the filename
        matches = []
        for f in Path.cwd().rglob(f"**/{path.name}"):
            # Skip if in ignored directories
            if any(part in {"__pycache__", ".git", "venv", ".venv", "node_modules"} for part in f.parts):
                continue
            matches.append(f)
    ...
    # Offer to launch picker
    try:
        ans = input("  Launch file picker to search? [Y/n]: ").strip().lower()
        if ans in ("", "y", "yes"):
            return pick_py_file()
    except EOFError:
        pass
...
def main():
    ...
    def resolve_file(file_arg):
        resolved = resolve_file_with_path(file_arg)
        if not resolved:
            sys.exit(1)
        return resolved
...
Examples:
  ...
  python pywa.py build ../projects/myapp.py  ← relative path
  python pywa.py build /home/user/myapp.py   ← absolute path
