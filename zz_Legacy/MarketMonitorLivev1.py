...
def fetch_yahoo_data(ticker, start_date, end_date, interval):
    ...
    for attempt in range(max_retries):
        try:
            ...
            print(f"HTTP Error {e.response.status_code if hasattr(e.response, 'status_code') else 'unknown'} for {ticker} ({interval})")
...
def run_analysis_and_serve(days_count):
    ...
    # ─── Last Known Prices (robust against trailing NaNs) ───
    last_idx = df.index[-1]  # anchor position (right edge)
    ...
    if not np.isnan(g_price):
        fig.add_annotation(
            ...
            xanchor="left", ax=35, ay=0,
            font=dict(color='#D4AF37', size=12),
            showarrow=False,
            row=1, col=1
        )
    ...
    if not np.isnan(s_price):
        fig.add_annotation(
            ...
            xanchor="left", ax=35, ay=0,
            font=dict(color='#B0C4DE', size=12),
            showarrow=False,
            row=1, col=1,
            secondary_y=True
        )
    ...
    if not np.isnan(d_price):
        fig.add_annotation(
            ...
            xanchor="left", ax=35, ay=0,
            font=dict(color='#4169E1', size=12),
            showarrow=False,
            row=2, col=1
        )
